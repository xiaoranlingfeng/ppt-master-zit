...
### 4.5 Local ComfyUI backend (`comfyui` / zit基础 · Z-Image Turbo)

When `IMAGE_BACKEND=comfyui`, images are generated by the local ComfyUI「zit基础」workflow (Z-Image Turbo). The backend auto-starts the aki-v3 integration when `127.0.0.1:8188` is not responding and opens the ComfyUI web UI in the default browser afterwards so the queue is visible (`COMFY_AUTOSTART=0` disables auto-start; `COMFY_OPEN_BROWSER=0` keeps it headless), submits the API-format template with the prompt patched in, and picks a fresh seed each run. Sampler defaults ship matched to the model (steps 8 / cfg 1 / res_multistep / simple / shift 3) and can be overridden via `COMFY_STEPS`, `COMFY_CFG`, `COMFY_SAMPLER`, `COMFY_SCHEDULER`, `COMFY_SHIFT` in `.env`; resolution is computed from the row's `aspect_ratio` × `image_size` (nearest multiple of 8).

Prompting rules for this backend (apply in addition to §5):

1. **Natural language, not tag soup.** Write one flowing descriptive sentence or short paragraph, Chinese or English, in this order: subject → scene/environment → lighting/mood → style/medium. Z-Image Turbo is trained on natural-language captions; comma-separated tag lists degrade composition.
2. **Length**: ~1–4 sentences (roughly 40–150 Chinese characters or 25–80 English words). Long essays dilute the subject.
3. **No text in the image.** Never ask for words, letters, numbers, logos, or labels; append the Chinese clause 「画面中不出现任何文字、字母、数字或水印」 (or English "no text, letters, numbers, or watermarks in the image") to every prompt. Text overlays belong to the SVG/PPTX layer, not the raster.
4. **Deck-fit styling.** State the rendering style explicitly (扁平插画 / 水墨 / 等距 3D / 摄影感 / 极简商务等) and keep backgrounds clean when the image will sit under captions or scrims.
5. **Reference-image edits** (`--reference-image`) use plain English edit instructions describing what to remove/isolate and what to preserve, same as §4.4.

Example (cover illustration, 16:9):
`现代教育主题概念插画：明亮的大学教室里，学生围着笔记本电脑协作讨论，窗外阳光洒入，简洁扁平插画风格，暖色调，画面干净留白，适合演示文稿配图，画面中不出现任何文字、字母、数字或水印。`
